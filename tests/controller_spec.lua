...
function M.run(test)
  ...
  test("VeryLazy restores the buffer-local toggle mapping", function()
    local buffer = vim.api.nvim_get_current_buf()
    vim.keymap.set("i", "<C-j>", "distribution-toggle", { buffer = buffer })
    vim.api.nvim_exec_autocmds("User", { pattern = "VeryLazy" })
    equal(vim.fn.maparg("<C-j>", "i", false, true).desc, "skk-lite: j")
  end)
  ...
  test("insert mappings are buffer-local and restore existing mappings", function()
    vim.cmd("enew!")
    local buffer = vim.api.nvim_get_current_buf()
    skk.setup({ mappings = false })
    vim.keymap.set("i", "a", "original-a", { buffer = buffer })
    vim.keymap.set("i", "<C-j>", "original-toggle", { buffer = buffer })

    skk.setup({ mappings = true })
    equal(vim.fn.maparg("a", "i"), "original-a")
    skk.enable("insert")
    equal(type(vim.fn.maparg("a", "i", false, true).callback), "function")
    skk.disable("insert")
    equal(vim.fn.maparg("a", "i"), "original-a")

    skk.setup({ mappings = false })
    equal(vim.fn.maparg("<C-j>", "i"), "original-toggle")
  end)

  test("command-line mappings restore existing mappings", function()
    vim.keymap.set("c", "w", "original-commandline")
    skk.setup({ mappings = true })
    skk.enable("cmdline")
    equal(type(vim.fn.maparg("w", "c", false, true).callback), "function")
    skk.disable("cmdline")
    equal(vim.fn.maparg("w", "c"), "original-commandline")
    skk.setup({ mappings = false })
    vim.keymap.del("c", "w")
  end)

  require("skk_lite.store").flush()
