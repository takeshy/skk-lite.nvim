...
function M.run(test)
  ...
  test("double n terminates n before vowels and at end of input", function()
    local overlap = harness()
    overlap.type("nna")
    equal(overlap.output(), "んあ")

    local terminal = harness()
    terminal.type("nn")
    equal(terminal.press("l").insert, "")
    equal(terminal.output(), "ん")
  end)

  test("funniki becomes funiki", function()
    local h = harness()
    h.type("funniki")
    equal(h.output(), "ふんいき")
  end)

  test("konnnichiha composes naturally with an explicit double n", function()
    local h = harness()
    h.type("Konnnichiha")
    h.press("Enter")
